...
--2.yol
select * from mezunlar where diploma_notu<=95 and diploma_notu>=75;
...
--SORU13: mezunlar tablosunu siliniz
...
update urunler set malzeme_isim='Telefon' where malzeme_isim='Phone';
...
update urunler set malzeme_isim=(select sirket_ismi from tedarikciler where irtibat_ismi='Sedat Koc')  
...
select sehir,round(avg(yas)) as ortalama_yas 
from manav group by sehir
...
select cinsiyet, round(avg(yas)) as ortalama_yas
from manav
group by cinsiyet
