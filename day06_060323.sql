...
select ulke, cinsiyet, count(isim) as calisan_sayisi, round (avg(yas)) as yas_ort from insan group by ulke,cinsiyet;
...
select ulke, sum(maas) as maas_top from insan
group by ulke 
...
union
select isim,maas,sirket from personel where not sirket='Honda'; 
--Not yerine 'sirket!'de kullanilabilir.
...
union
select isim,sehir from personel where not sehir='Istanbul'
